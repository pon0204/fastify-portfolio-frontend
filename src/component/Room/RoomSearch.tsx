...
const RoomSearch: VFC<Props> = ({ match }) => {
  const { status, data } = useQueryRoom(match.params.roomId)
  ...
  if (status === 'loading')
    return (
      <div className='absolute bottom-1/2 left-44 w-10 h-10 -translate-x-1/2 -translate-y-1/2 md:bottom-1/2 md:left-1/2'>
        <CircularProgress />
      </div>
    )
